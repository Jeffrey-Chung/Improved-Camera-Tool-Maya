...
def basicDepthofFieldSettings():
    ...
    return shotCameraShape, distanceDimensionShape
    

#add DOF rig via Focal length         
def addDepthofField():
    shotCameraShape, distanceDimensionShape = basicDepthofFieldSettings()
...
def addDepthofFieldFStop():
    shotCameraShape, distanceDimensionShape = basicDepthofFieldSettings()
    cmds.connectAttr(distanceDimensionShape + '.distance', shotCameraShape+'.fStop') #connect distance attribute of distance dimension to f stop of camera so that DOF can be varied
...
class cameraTools():
    ...
    def drawUI(self):
        ...
        cmds.button(label = 'Enable DOF Focal Length', command = 'addDepthofField()')
        cmds.separator(h=10)
        cmds.button(label = 'Enable DOF f Stop', command = 'addDepthofFieldFStop()')
        cmds.separator(h=20)
